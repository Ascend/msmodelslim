...
| dump_data_dir | Specifies the calibration data directory.| Specifies the directory for retrieving and saving calibration data. An empty string defaults to the model weight save path. If `calib_data.pth` exists in the specified directory, it is directly loaded as the calibration data. If `calib_data.pth` does not exist, the program automatically saves and loads the calibration data by using the dump mechanism. **This parameter takes effect only when `enable_dump` is set to `True`.**| A directory string.|

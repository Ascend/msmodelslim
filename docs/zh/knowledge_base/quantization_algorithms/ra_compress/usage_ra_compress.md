...
- 已确认校准集 tokenize 后总长度 ≥ 2500 × 4 = 10000 tokens（推荐使用内置 `calib_dummy.jsonl` 即可满足）。
...
2. 使用内置校准集 `calib_dummy.jsonl`（推荐，须在命令行通过 `--calib_dataset calib_dummy.jsonl` 显式指定）；若使用自定义校准集，需保证 tokenize 后总长度 ≥ 2500 × 4 = 10000 tokens，且段边界严格对齐。
...
| `--calib_dataset` | 校准数据集路径，`attn_head` 场景须显式指定为 `calib_dummy.jsonl`（合成重复段校准集，tokenize 后总长度 ≥ 10000） |

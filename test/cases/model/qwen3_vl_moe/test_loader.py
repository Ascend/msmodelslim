...
-------------------------------------------------------------------------
msmodelslim.model.qwen3_vl_moe.loader 模块的单元测试
"""
...
from msmodelslim.model.qwen3_vl_moe.loader import Qwen3VlMoeAdapterLoader
from msmodelslim.utils.exception import UnsupportedError


class TestQwen3VlMoeAdapterLoader:
    """测试 Qwen3VlMoeAdapterLoader 类"""

    def test_adapter_class_path_point_to_qwen3_vl_moe_adapter_when_defined(self):
        """ADAPTER_CLASS_PATH 应指向 Qwen3VLMoeModelAdapter"""
        assert (
            Qwen3VlMoeAdapterLoader.ADAPTER_CLASS_PATH
            == "msmodelslim.model.qwen3_vl_moe.model_adapter:Qwen3VLMoeModelAdapter"
        )

    def test_load_return_adapter_instance_when_class_path_valid(self):
        """load 在类路径有效时应返回适配器实例"""
        loader = Qwen3VlMoeAdapterLoader()
        ...
        with patch("msmodelslim.model.plugin_factory.base_loader.import_module") as mock_import:
            mock_import.return_value = SimpleNamespace(Qwen3VLMoeModelAdapter=DummyAdapter)
            with patch("msmodelslim.model.plugin_factory.base_loader.DependencyChecker.set_plugin"):
                with patch("msmodelslim.model.plugin_factory.base_loader.DependencyChecker.check_plugin"):
                    result = loader.load(
                        model_type="Qwen3-VL-MoE",
                        model_path=Path("/tmp/model"),
                        trust_remote_code=True,
                    )

        assert isinstance(result, DummyAdapter)
        assert result.model_type == "Qwen3-VL-MoE"
    ...
    def test_load_raise_unsupported_when_adapter_class_path_invalid(self):
        ...
        loader = Qwen3VlMoeAdapterLoader()
        loader.ADAPTER_CLASS_PATH = "invalid_path_without_colon"

...
from msmodelslim.core.quantizer.impl.dualscale import MXWeightDualScaleMinmax, MXActDualScaleMinmax, MXFP4_MAX_NORMAL
...
class TestMXWeightDualScaleMinmax:
    ...
    def test_initialization_raise_schema_validate_error_when_invalid_axes_type(self):
        ...
        invalid_config = QConfig(
            dtype="mxfp4", scope="per_block", method="dualscale", symmetric=True,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                torch.cat([
                    torch.linspace(-12.0, 12.0, 512).unsqueeze(0),
                    torch.linspace(-24.0, 24.0, 512).unsqueeze(0)
                ], dim=-1),
    ...
    def test_dual_scale_calculate_correct_scale_when_init_weight_with_boundary_values(
        ...
        quantizer.inner_quantizer.get_q_storage = MagicMock(
            return_value=QStorage(QDType.FLOAT, weight_tensor)
        )

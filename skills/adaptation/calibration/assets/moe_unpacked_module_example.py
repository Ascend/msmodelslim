...
class TopKRouter(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        logits = torch.nn.functional.linear(hidden_states, self.weight)  # pylint: disable=not-callable
...
class SparseMoeBlockWithLinearExperts(nn.Module):
    ...
    def __init__(
        ...
        self.experts = nn.ModuleList([MoeExpertMLP(hidden_size, intermediate_size, act_fn) for _ in range(num_experts)])

...
- `model_adapter.py`：适配器入口与 5个必需接口实现
...
- **必须实现**（5个）：`handle_dataset`、`init_model`、`generate_model_visit`、`generate_model_forward`、`enable_kv_cache`
...
- `assets/moe_unpacked_module_example.py`
- `assets/moe_unpacked_adapter_example.py`

## 可选高阶特性

逐层量化（按层加载/懒加载）相关实现、工作流与示例见独立 Skill `adaptation/calibration/layer_wise`。

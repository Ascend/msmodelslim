...
import argparse
import os
import subprocess  # nosec B404 - 执行 msmodelslim quant 是 skill 的本质需求
import sys
...
def main():
    ...
    cmd = [
        "msmodelslim",  # console script：本包无顶层 __main__.py，不能用 python -m msmodelslim
        "quant",
        "--model_path",
        args.model_path,
        "--save_path",
    ...
    rc = subprocess.run(cmd, check=False).returncode  # nosec B603 - 命令为固定参数列表，无 shell 注入面
    if rc != 0:
        print("[ERROR] step2失败")
        return rc

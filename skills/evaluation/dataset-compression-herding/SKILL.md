---
name: dataset-compression-herding
description: |
...
metadata:
  ...
  # 独立数据准备任务：无适配产物、无验收门禁；仅在用户无自备压缩数据集且同意生成时委派
  subagent:
    id: "evaluation/dataset-compression-herding"
    bind:

      - "evaluation/dataset-compression-herding"

---

# 数据集压缩（dataset-compression-herding）
...
- **用户自备已压缩数据集**：直接跳过第 2、3步，进入第 4步生成测试脚本。
- **用户不自备**：继续第 2步。
...
- 场景 A（仅压缩测评）：第 4步生成的「子集测试脚本」应指向该 `_coreset` 目录，「全集测试脚本」指向原始 `<dataset_name>/` 目录。
- 场景 B（接入量化调优）：见下方小节，产出 coreset 数据集配置与 config_name。

### 产出 coreset 数据集配置（场景 B：接入量化调优主流程）

当压缩结果用于量化调优的快速迭代时，交付物不是 shell 脚本，而是**一套可被 evaluation.yaml 直接引用的 coreset 数据集配置**。编排层（`tuning`）通过切换 evaluation.yaml 中 `datasets.<key>.config_name` 在「全集 / coreset」之间切换，全程复用 msmodelslim 内建评测服务（`run_evaluation.py` → `ServiceOrientedEvaluateService`），不新增任何服务拉起脚本。
...
|------|------|------|------|------|------|------|
| E-001 | 压缩耗时与建议自备 | 执行压缩前 | 现有实现压缩约 30分钟（AIME/GPQA）且大量 CPU numpy 存在性能瓶颈；**优先建议用户自备已压缩数据集** | 三·第 2步 | 用户反馈 | 已回归 |
| E-002 | 支持范围边界 | 任意压缩请求 | 暂时仅支持 `aime2025` / `gpqa`；其他数据集须结合 aisbench `herding_coreset_selector` 源码分析，**不复制/修改 herding 算法** | 概述 | 用户反馈 | 已回归 |
| E-003 | 不自动安装下载 | 环境检查阶段 | AISBench 代码下载、安装、依赖安装均须先征得用户确认，不自动执行；`pip list` 无 `aisbench` 时要求用户先安装 | 一·步骤 2 | 用户反馈 | 已回归 |
| E-004 | 交付二选一 | 压缩完成或用户自备后 | 先确认用途再选一种交付（场景 A 全集/子集测试脚本 / 场景 B coreset 配置 + config_name），不同时产出两种 | 三·第 4步 | 用户反馈 | 待验证 |
| E-005 | coreset 不改评测层 | 场景 B 接入调优主流程 | `run_evaluation.py`/服务只认 `config_name` 不认数据路径；切子集只能在 ais_bench 数据集配置层做；`<原config_name>_coreset` 配置复制原 `.py` 仅改 `path` | 四·产出 coreset 配置 | 代码实证 | 已回归 |

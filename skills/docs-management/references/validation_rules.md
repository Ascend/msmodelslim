...
按文档类型对照 `docs/zh/contributing/development_guide/docs_standards/` 下对应校验清单执行校验。**新增场景校验 = 场景特有校验清单（若配置）+ 文档类型清单 + [《公共校验清单》](../../../docs/zh/contributing/development_guide/docs_standards/00_common_checklist.md)，分别执行并综合结果**（见 `scenarios/` 场景文档）。场景特有校验清单仅含该场景特有、文档类型清单未覆盖的条目，不重复类型清单。
...
- **类型校验清单**：`NN_*_checklist.md`，类型专属条目。
...
| 文档类型 | 类型校验清单 |
| --- | --- |
| 算法词条 / 术语词条 | [《量化术语百科校验清单》](../../../docs/zh/contributing/development_guide/docs_standards/01_quantization_glossary_checklist.md) |
...
| 模板体系外（安装指南 / 快速入门 / FAQ 等） | 无类型清单，以[《公共校验清单》](../../../docs/zh/contributing/development_guide/docs_standards/00_common_checklist.md)的 ERROR 条目（CE-01~CE-04）为兜底 |
...
- **CE-02（模板注释）**：覆盖模板自带的教学性 / 脚手架章节（如 01 词条的「排版规范」），不仅限 `> **注释：**` 块引用。发布文档中此类"指导作者写作"的模板指令章节应删除；确属发布内容需保留的，须改写为正式说明并去除占位示例。判定依据见[《公共校验清单》](../../../docs/zh/contributing/development_guide/docs_standards/00_common_checklist.md) CE-02。
...
先公共后类型：CE 条目优先；再按类型清单编号顺序修复（词条：G > S > L > C > P > R > K；流程 PG 序；案例 GC 序；配置 Q 序；CLI C 序）。通用与结构问题优先，避免后续条目连锁失败。
...
```text
<!-- waiver: P02 原因：纯概念术语无公式 -->
```

校验时跳过对应条目并记录豁免原因。豁免不得用于规避 ERROR 类条目超过 2 项，WARN 条目不设豁免数量限制。

## 词条待建 TODO 登记（统一动作，K02 / L04）
...
校验时：已登记待建词条豁免 K02（ERROR）与 L04（WARN）的存在性检查；未登记则报告 ERROR，提示登记或修正链接。不设中心化待建清单。

**与 L09 的分工**：L09（链接路径可解析，ERROR）只校验相对路径能否在仓库内解析——待建词条链接路径合法即满足，与待建登记无关。目标文件**存在性**由 K02 / L04 管：已登记 TODO 的待建词条链接，其目标文件不存在属"已规划"，K02 通过、L04 豁免，仅记录待建，**不**报 L09 存在性类 ERROR。

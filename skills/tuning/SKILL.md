---
name: tuning
description: End-to-end automated model quantization and accuracy tuning workflow. Use when user asks for automated model quantization and accuracy tuning, e.g. "自动量化", "量化调优", "一键量化", "精度调优", etc. 当用户给出多卡（≥2卡）卡号时，量化阶段通过 EP 并行适配（adaptation/ep）保证多卡 EP 并行，调优主流程仍在本 Skill。
license: Apache-2.0
metadata:
  version: 0.9.5
  domain: quantization
...
关于相关业务的背景知识，如什么是modelslim、什么是量化精度调优等，可以参考[量化自动调优背景知识](references/background_information.md)。业务概念与产品级使用流程以 docs 为准：《[自动调优使用指南](../../docs/zh/user_guide/usage_auto_precision_tuning.md)》、《[自动调优策略总览](../../docs/zh/knowledge_base/tuning_strategies/README.md)》。
...
当用户在设备索引处给出**多卡卡号**（≥2 张卡，如 `--device npu --device_id 0 1`、`--device_id 2 3`、`[0,1,2,3]`）时，本 Skill **主流程保持不变**，仅在量化前委派 **EP 并行适配** Skill（`adaptation/ep`），保证后续量化全程以 EP 并行进行：

- **触发条件**：用户给出的设备卡号数量 ≥ 2。
- **动作**：在「模型准备 → 量化配置调优」之间，委派 `adaptation/ep` 完成 **MoE 检查 + EP 就绪检查与适配 + `[EP_CHECK]` 验证**，并回传 `EP_ADAPT_RESULT` 与 `requires_ep`。本 Skill 据此决定后续量化是否走 EP：
  - `requires_ep=true`（MoE，EP 已适配）→ **后续调优全程开启 EP 并行**：每一轮量化命令固定使用多卡（`--device npu --device_id 0 1 ...`），每轮量化日志均须含 `[EP_CHECK]`，评测服务同样保持多卡，中途不得退回单卡 / DP；
...
结构化回退经验（`strategy/standing_high_with_experience/expert-rules`）不属 EP 适配范围：当调优策略为 `standing_high_with_experience` 时，在量化配置调优阶段、进入二分搜索前委派它取得「哪些层需要回退」的结构化意见，作为 practice-generator 生成 Practice 的初值（见[量化配置调优](./references/quantization_tuning.md)「结构化回退经验」）。
...
委派 subagent 时须遵守 [主↔子交互协议 SUBAGENT_IO v1](./references/subagent_io_protocol.md)。
...
量化配置调优完成后，按以下规范整理并输出最终调优结果（详细规范见《[输出格式](./references/output_format.md)》）：

- **交付物**：量化后的模型文件（`{save_path}/round_{N}/quantized` 中的达标权重）、评测报告（各数据集精度分数与目标对比）、调优历史记录（`{save_path}/history`）。
- **回显格式**：向用户展示最终达标轮次、模型产物路径、各数据集精度 vs 目标、总耗时；附上可复现的量化/评测命令摘要。
- **收敛确认**：回显后须获得用户认可，调优流程才算圆满完成；用户有异议时按反馈补充或调整，直至用户满意。
- **Practice 入库**：收敛确认后调用 `tuning/scripts/finalize_practice_repo.py` 将最终 Practice 写入实践库（`--model-type` / `--model-path` / `--final-practice-path`），便于后续 `msmodelslim quant` 复用（详见 `quantization_tuning.md`「调优结束与 Practice 入库」）。
...
通过命令执行方式调用，路径相对于仓库 `skills/` 根目录（或先定位 skill 根目录再拼接 `scripts/`）：

| 脚本 | 用途 |
|------|------|
| `tuning/scripts/history_clear.py` | 每轮循环开始前清空 history |
| `tuning/scripts/accuracy_lookup.py` | 量化/评测前查精度缓存 |
| `tuning/scripts/accuracy_append.py` | 评测后写精度缓存 |
| `tuning/scripts/history_append.py` | 每轮结束后追加调优历史 |
| `tuning/scripts/accuracy_cleanup.py` | 可选，手动清理 accuracy 缓存 |
| `tuning/scripts/finalize_practice_repo.py` | 调优收敛后写入 practice 仓库 |

子步骤见对应 Skill：`strategy/practice-cfg`（Practice 生成 + 校验脚本；其步骤②再委派 `sensitive-layer-analysis` 完成 `msmodelslim analyze`）、`quantization`（`msmodelslim quant`）、`evaluation/evaluate`（评测脚本）；结构化回退意见见 `strategy/standing_high_with_experience/expert-rules`（`standing_high_with_experience` 策略二分前委派）。

**压缩数据集（默认）**：默认的量化调优过程均使用压缩数据集进行快速迭代。进入循环前**必须**向用户确认来源，三选一：①用户自备已压缩数据集；②委派 `evaluation/dataset-compression-herding` skill 用 RBF Kernel Herding 生成 coreset（仅支持 `aime2025`/`gpqa`，耗时约 30分钟，已获用户确认后执行）；③用户两者都不愿意时退回全集测试。主流程采用「**子集调优 → 全集验证 → 不通过改全集调优**」：先用子集调优直到子集达标，再全集验证，全集不达标则**直接切换到全集进行调优**，保证最终与全集一致；**不再**采用固定步长逐步收紧子集出口标准的容忍性做法。进入循环前须先确定**子集与全集两个出口标准**：先询问用户是否分别给出，不给出的一方在当前环境跑浮点模型测 FP 基线（**浮点基线评测会额外占用卡数，须向用户提示并确认可用卡**）。详见 `references/quantization_tuning.md` 的「压缩数据集的使用」。
...
> 追加规范见 `skills/README.md`「经验条目」。连续编号 `[E-序号]`；正文保留权威展开，本表只做索引 + 元数据登记；来源：实测编号（F0-x / Dx / Tx）| 用户反馈 | 代码实证；验证状态：已回归 | 待验证 | 已上流 docs。本 skill 为**编排层**，仅登记编排级原则；子任务操作级经验登记在各被委派子 skill。

| 条目 | 主题 | 适用条件 / 触发信号 | 结论要点（一句话） | 正文位置 | 来源 | 验证状态 |
|------|------|------|------|------|------|------|
| E-001 | 磁盘管理上限 | 任意调优过程 | 同时最多保留 2份完整权重（当前迭代 + 已达标最优一轮）；删除**严禁** `rm -rf`，一律 `rm -r` | 红线与原则-磁盘管理 | 用户反馈 | 已回归 |
| E-002 | 官方 CLI / 脚本原则 | 执行分析/量化/评测 | 分析/量化走 `msmodelslim analyze`/`msmodelslim quant`，编排校验/评测走文档指定脚本；以 exit code / stdout JSON 判成败，禁止伪造输出或跳过 Skill 步骤 | 红线与原则 | 用户反馈 | 已回归 |
| E-003 | 错误即停不上层续跑 | 命令失败后 | 未预见错误立即中止，报命令名 + 错误摘要；不换未文档化命令规避、不修改源码规避（属执行范围红线） | 红线与原则-排障和兜底 | 用户反馈 | 已回归 |
| E-004 | 子集/全集双出口 | 进入调优循环前 | 「子集调优 → 全集验证 → 不通过改全集调优」，子集与全集是两个独立出口标准；先问用户是否分别给出，未给的一方跑 FP 基线（额外占用卡，须向用户提示并确认） | 常用脚本-压缩数据集 | 实测 F0-7 | 已回归 |

...
执行全自动量化调优前，Agent 从自然语言中**自动提取**必要参数；用户无需手写完整 YAML。落地后的 practice/评测仍须走本 Skill 的 **YAML 校验与编排**（与主 `SKILL.md` 一致）。
...
| 调优历史记录路径 | 用户指定优先，否则 `{save_path}/history`（与编排脚本 `tuning/scripts/history_*.py` 实际读写路径一致）；**新任务**与**续跑**须与用户确认是否清空或沿用 |
| **权重量化位数**（w_bit） | "8bit"/"int8"→8，"4bit"/"int4"→4；**未提及时常用 8**（与 `strategy/references/practice_yaml_format.md` 的量化模板一致） |
| **激活量化位数**（a_bit） | 常与 w_bit 一致，除非用户指定；组合见 `strategy/references/practice_yaml_format.md` |
...
> 「精度损失不超过 2%」 → 容差与基准精度结合；若无基准精度须引导评估或放宽假设（参照 `practice_yaml_format.md` 中 `demand` 的精度目标定义）。
...
**无明确指定时的决策规则**（策略原理详见《[自动调优策略总览](../../../docs/zh/knowledge_base/tuning_strategies/README.md)》）：

| 用户掌握的信息 | 推荐策略 |
|----------------|----------|
| 已有完整量化 YAML（固定离群值抑制与量化模板） | `standing_high`（或 `binary_fallback`） |
| 仅知量化类型（w8a8/w4a8）与模型结构 | `standing_high_with_experience` |
| 用户明确指定策略名 | 以用户指定为准 |

决策后须在参数回显中写明所选策略与理由，经用户确认后再进入下一步。

## 处理流程

1. **提取参数**：从自然语言中提取可确定项
2. **补全缺失**：必填缺失时渐进式提问
3. **推导配置**：按 `practice_yaml_format.md` 的 schema 形成可校验的 practice / 评测意图
4. **确认参数**：**回显**全部关键项（含默认值）并经用户确认后再进入调优循环
5. **执行调优**：与主 SKILL 一致：校验 → 缓存 → 量化 → 评测 → history

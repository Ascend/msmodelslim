...
详细安装步骤见 skill [installation](../../installation/SKILL.md) 与《[安装指南](../../../docs/zh/install_guide/install_guide.md)》。端到端调优的环境准备全流程见《[自动调优使用指南](../../../docs/zh/user_guide/usage_auto_precision_tuning.md)》。下列为编排侧检查要点。
...
> **源码目录内安装的注意（实测 F0-2）**：若执行过 `bash install.sh` 源码安装，验证 `msmodelslim` CLI / 运行 `msmodelslim analyze|quant` 时，**工作目录必须在仓库根目录之外**（如调优工作目录 `{save_path}`）。源码树内 import 会命中无 `config` 子目录的源码包，报 `SecurityError: msmodelslim/config doesn't exist` 假失败。另 CLI 首启较慢（约 20~60s），勿误判卡死。

1. 若模型适配阶段执行过 `bash install.sh`，脚本会在**同一 Python 环境**中读取新 adapter，**无需**重启工具服务；继续调优前确认 `import` 无报错即可。
...
1. 运行 `npu-smi info` 获取全部卡信息，确认**总卡数**（不可假设为 8卡，不同机器卡数不同）
...
```bash
python skills/tuning/scripts/history_clear.py --save-path /path/to/workdir
```

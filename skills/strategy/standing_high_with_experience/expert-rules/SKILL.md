...
metadata:
  ...
  subagent:
    id: "strategy/standing_high_with_experience/expert-rules"
    bind:

      - "strategy/standing_high_with_experience/expert-rules"
...
按「通用 → 模型类 → 专属」递进，**新模型先通读 L1，再按触发信号读 L2 对应小节，最后按 vendor 查 L3个案**。
...
3. 命中有 L3个案的模型，叠加 `models/<vendor>/<model>.md`。
...
| 校准集 | 10–50条、场景匹配、删异常、加 badcase | 第 4步。校准数据不足或分布偏移会导致量化参数估计偏差 |
...
- 协同执行：`adaptation/ep`（EP 适配）；调优主流程由 `tuning` 承接

## 经验条目（Experiences，追加制）

> 本 skill 即**全体系经验的检索索引与专家库载体**：经验的"三级知识结构"（L1 通用 → L2 结构族 → L3 模型个案）即为本 skill 的可追加条目机制，登记规则见 `skills/README.md`「经验条目」适用层表格与本节「三级知识结构」。新增经验按以下规则归位，**不在此处堆叠表格**：
>
> - 跨模型通用经验 → 追加/修订 `cross-model-pitfalls.md`（L1）；
> - 某类结构（MoE / MLA / VLM / DSA 等）专属 → `structure-family-pitfalls.md`（L2）；
> - 某模型/厂商个案 → `models/<vendor>/<model>.md`（L3），并更新 `models/README.md` 索引；
> - 每条结论保持「专家原因 + 可信度」，来源/验证状态写入条目内。

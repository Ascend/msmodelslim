...
def _pack_git_branch() -> str:
    """Resolve the branch being packed so installed wheels still know their origin."""
    branch = os.environ.get('CI_COMMIT_REF_NAME') or os.environ.get('GITHUB_REF_NAME') or ''
    if not branch:
        branch = _git_output('rev-parse', '--abbrev-ref', 'HEAD')
    if branch and branch != 'HEAD':
        return branch
    named = _git_output('name-rev', '--name-only', '--exclude', 'remotes/*', 'HEAD')
    if not named or named in ('HEAD', 'undefined'):
        named = _git_output('name-rev', '--name-only', 'HEAD')
    if not named or named in ('HEAD', 'undefined'):
        return ''
    named = named.replace('remotes/origin/', '').replace('remotes/', '')
    return named.split('~', 1)[0].split('^', 1)[0]


def _write_build_info(path: str) -> None:
    """Write pack-time git hash, branch and local timestamp into the wheel/sdist."""
    git_hash = _git_output('rev-parse', 'HEAD')[:12]
    git_branch = _pack_git_branch()

...
from msmodelslim.ir import mxfp4_dual_scale_sym
...
@QABCRegistry.multi_register(dispatch_key=[(mxfp4_dual_scale_sym, mxfp4_dual_scale_sym)], abc_type=AutoFakeQuantLinear)
@logger_setter()
class W4A4MXDynamicDualScaleFakeQuantLinear(AutoFakeQuantLinear):
    def __init__(self, x_q_param: QParam, w_q_param: QParam, w_q: QStorage, bias: torch.Tensor):
        ...
        self.w_target_axes = None
        self.x_target_axes = None
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        dual_scale_x = (dual_block_max_val / MXFP4_MAX_NORMAL).clamp(min=1e-5)
        ...
        x_dualscaled, axes_, orig_shape, padded_shape = reshape_to_blocks(
            x_dualscaled, self.x_target_axes, self.x_inner_block_size
        )
        ...
        x_dualscaled_q_dq, axes_, orig_shape, padded_shape = reshape_to_blocks(
            x_dualscaled_q_dq, self.x_target_axes, self.x_dual_block_size
        )
        ...
        weight_dualscaled, axes_, orig_shape, padded_shape = reshape_to_blocks(
            weight_dualscaled, self.w_target_axes, self.w_inner_block_size
        )
        weight_dualscaled_q_dq = dequantize(
            QStorage(self.w_scheme.dtype, weight_dualscaled), self.inner_w_q_param
        ).value
        weight_dualscaled_q_dq = undo_reshape_to_blocks(
            weight_dualscaled_q_dq, padded_shape, orig_shape, self.w_target_axes
        )

        # wei de_quant - dual_scale
        weight_dualscaled_q_dq, axes_, orig_shape, padded_shape = reshape_to_blocks(
            weight_dualscaled_q_dq, self.w_target_axes, self.w_dual_block_size
        )

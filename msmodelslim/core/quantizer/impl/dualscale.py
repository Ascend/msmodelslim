...
class MXWeightDualScaleMinmax(AutoWeightQuantizer):
    def __init__(self, config: QConfig):
        ...
        self.inner_w_q_param = None
        self.inner_w_q_storage = None
    ...
    def forward(self, x: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if dual_scale is None:
            raise UnexpectedError(
                "The parameter 'dual_scale' cannot be None.",
                action="Please ensure that 'dual_scale' is correctly initialized or passed.",
            )

        inner_dequant_value, axes_, orig_shape, padded_shape = reshape_to_blocks(
            inner_dequant_value, axes_, self.dual_block_size
        )
    ...
    def init_weight(self, weight: QStorage, bias: Optional[torch.Tensor] = None) -> None:
        ...
        weight_value, axes_, orig_shape, padded_shape = reshape_to_blocks(
            weight_value, self.target_axes, self.dual_block_size
        )
        dual_scale_axes = [x + 1 for x in axes_] if self.dual_block_size > 0 else axes_
        self.minmax_block_observer.update(
            weight_value.to(torch.float32), sync=self.sync, shared_exp_axes=dual_scale_axes
        )
        dual_block_min_val, dual_block_max_val = self.minmax_block_observer.get_min_max()
        dual_scale = (dual_block_max_val / MXFP4_MAX_NORMAL).clamp(min=1e-5)
        dual_scale_weight_value = weight_value.to(torch.float32) / dual_scale
        dual_scale_weight_value = undo_reshape_to_blocks(
            dual_scale_weight_value, padded_shape, orig_shape, self.target_axes
        )
    ...
    def get_q_storage(self) -> QStorage:
        if not self.w_q_storage:
            raise UnexpectedError(
                "The parameter 'self.w_q_storage' cannot be None.",
    ...
    def get_q_param(self) -> QParam:
        if not self.w_q_param:
            raise UnexpectedError(
                "The parameter 'self.w_q_param' cannot be None.",
                action="Please execute 'self.init_weight' first to initialize the quantization parameters.",
            )

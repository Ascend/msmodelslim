...
class Step3p5RotaryEmbedding(nn.Module):
    def __init__(self, config, device=None, layer_idx=None):
        ...
        # Use a per-instance config copy so this layer does not mutate the shared
        # config object — otherwise the next MTP layer (idx+1) would see the
        # previous layer's scalar rope_theta and use the wrong base frequency.
        self.config = copy.copy(config)
        if self.config.rope_parameters is not None:
            self.config.rope_parameters = copy.deepcopy(config.rope_parameters)

        partial_rotary_factors = getattr(self.config, "partial_rotary_factors", None)
        if partial_rotary_factors is not None:
            self.config.partial_rotary_factor = partial_rotary_factors[0 if self.layer_idx is None else self.layer_idx]
        else:
            self.config.partial_rotary_factor = 1.0

        self.rope_theta = self.config.rope_theta
        if isinstance(self.rope_theta, list):
            self.rope_theta = self.rope_theta[0 if self.layer_idx is None else self.layer_idx]
        self.config.rope_theta = self.rope_theta
...
class Step3p7MTPModule(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        # Per-layer SwiGLU clamp limit. The official decoder reads
        # `config.swiglu_limits[layer_idx]`; MTP layer indices are 45/46/47 and
        # currently all 0 in the shipped config, but we keep parity so that a
        # future config with non-zero MTP limits is handled correctly.
        mtp_swiglu_limit = None
        mtp_swiglu_limits = getattr(config, "swiglu_limits", None)
        if (
            mtp_swiglu_limits is not None
            and mtp_swiglu_limits[layer_idx] is not None
            and mtp_swiglu_limits[layer_idx] != 0
        ):
            mtp_swiglu_limit = mtp_swiglu_limits[layer_idx]

        self.mlp = Step3p5MLP(config, config.intermediate_size, swiglu_limit=mtp_swiglu_limit)
        ...
        # ``attention_type`` mirrors Step3p7DecoderLayer.__init__
        # (modeling_step3p7.py:772-778) so the adapter's ``generate_model_forward``
        # can index the right causal mask from ``causal_mask_mapping``. The
        # shipped Step-3.7 config marks all three MTP slots (45/46/47) as
        # ``"sliding_attention"``; the fallback below matches the decoder-layer
        # rule used for any future configs without ``layer_types``.
        layer_types_cfg = getattr(config, "layer_types", None) or []
        if layer_types_cfg:
            self.attention_type = layer_types_cfg[layer_idx]
        else:

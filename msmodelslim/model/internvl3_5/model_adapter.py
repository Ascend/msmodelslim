...
from msmodelslim.model.internvl3_5.internvl_utils import load_image


@logger_setter()
class InternVL3_5ModelAdapter(  # pylint: disable=R0901
    VLMBaseModelAdapter,
    ModelInfoInterface,
    ModelSlimPipelineInterfaceV1,
    FlexSmoothQuantInterface,
    ...
    def handle_dataset(self, dataset: Any, device: DeviceType = DeviceType.NPU) -> List[Any]:
        ...
        get_logger().info("Loaded tokenizer for InternVL3.5")
        ...
        processed_data = []
        for item in tqdm(dataset, desc="Processing InternVL3.5 calibration dataset"):
            text = item.text
        ...
        get_logger().info("Processed %d samples for InternVL3.5", len(processed_data))
        return processed_data

    def init_model(self, device: DeviceType = DeviceType.NPU) -> nn.Module:
        get_logger().info("Initializing InternVL3.5 model with layer-wise loading...")
        ...
        get_logger().info("Loading InternVL3.5 model...")
        try:
            ...
            raise InvalidModelError(
                f"Failed to load InternVL3.5 model: {e}", action="Check model directory and trust_remote_code setting"
            ) from e
        ...
        # CRITICAL: Copy llm_config attention heads to model.config for OV smoothing
        # BaseSmoothProcessor._apply_standard_ov_smooth() reads from model.config
        ...
        get_logger().info(
            "Model initialized with %s layers (1 loaded, others will be loaded on-demand)",
            origin_layers,
        )
    ...
    def generate_model_forward(self, model: nn.Module, inputs: Any) -> Generator[ProcessRequest, Any, None]:
        ...
        if pixel_values is not None:
            pixel_values = pixel_values.to(dtype=model.vision_model.embeddings.patch_embedding.weight.dtype)
    ...
    def _load_decoder_if_not_exist(self, model: nn.Module, name: str, idx: int) -> nn.Module:
        ...
        with patch.object(nn.Linear, 'reset_parameters', lambda _self: None):
            ...
            decoder = Qwen3DecoderLayer(self.config.llm_config, layer_idx=idx)
    ...
    def get_adapter_config_for_subgraph(self) -> List[AdapterConfig]:
        ...
        for layer_idx in range(num_layers):
            ...
            post_layernorm_mapping = MappingConfig(
                source=f"language_model.model.layers.{layer_idx}.post_attention_layernorm",
                targets=[
                    f"language_model.model.layers.{layer_idx}.mlp.gate_proj",
                    f"language_model.model.layers.{layer_idx}.mlp.up_proj",
                ],
            )
            ...
            up_down_mapping = MappingConfig(
                source=f"language_model.model.layers.{layer_idx}.mlp.up_proj",
                targets=[f"language_model.model.layers.{layer_idx}.mlp.down_proj"],
            )

            adapter_config.extend(
                [
                    AdapterConfig(subgraph_type="norm-linear", mapping=input_layernorm_mapping),
                    AdapterConfig(subgraph_type="norm-linear", mapping=post_layernorm_mapping),
    ...
    def get_rotate_map(self, block_size):
        ...
        # 2. Rotate visual merger output projections (mlp1) to match rotated text embeddings
        # mlp1 has multiple linear layers, we need to rotate the final output projection
        # For InternVL3_5, mlp1 is a sequential: [LayerNorm, Linear, GELU, Linear]
        # We need to rotate the final Linear layer (index 3) with left rotation
        ...
        for layer_idx in range(config.num_hidden_layers):
            ...
            right_rot[f"language_model.model.layers.{layer_idx}.mlp.gate_proj"] = rot
            right_rot[f"language_model.model.layers.{layer_idx}.mlp.up_proj"] = rot
            left_rot[f"language_model.model.layers.{layer_idx}.mlp.down_proj"] = rot
